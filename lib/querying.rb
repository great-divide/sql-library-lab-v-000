...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(name) FROM characters GROUP BY species ORDER BY species DESC LIMIT 1"
end
...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters ON series.id = characters.series_id GROUP BY characters.species HAVING characters.species = 'human' ORDER BY COUNT(characters.species) DESC LIMIT 1"
end
